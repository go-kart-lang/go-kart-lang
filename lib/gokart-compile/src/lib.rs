...
use gokart_core::{Exp, ExpNode, GOpCode, Label, OpCode, Pat, PatNode, Sys, Var};
use rpds::List;
...
fn transform_postponed_label(postponed_labels: &Vec<u32>, l: i32) -> Label {
    if l < 0 {
        let idx = (-l - 1) as usize;
        postponed_labels[idx] as Label // todo
    } else {
        l as Label
    }
}
